...
def train(audio_model, train_loader, test_loader, args):
    ...
    print('scheduler for movie sounds is used')
    ...
    while epoch < args.n_epochs + 1:
        ...
        for i, (audio_input, clip_feats, labels) in enumerate(train_loader):
            ...
            with autocast():
                audio_output = audio_model(audio_input, clip_feats)
        ...
        mAP = np.mean([stat['AP'] for stat in stats if not np.isnan(stat['AP'])])
...
def validate(audio_model, val_loader, args, epoch):
    ...
    A_loss = []
    with torch.no_grad():
        for i, (audio_input, clip_feats, labels) in enumerate(val_loader):
